...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer - It has the zipcodes and the names of where the zipcode is for Maryland, along with "object ID", the length and area and the longitutde and latitiude (or at least I belueve that's what the geometry section is with the multipolygon portion). **
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer - it has zip codes, zip code names (which county they belong to), longitutde and lattitude in the geometry section, etc**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it is a map of Maryland and each zip code in the state**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it loads tigris, which is a package that you can use to download a shapefile that shows the outline of each Maryland county from the U.S. Census**
...
**Task** Run the following code. Describe the output in the space below. **Answer - It shows specifics like the different state and county names, FIPS codes, longitudes and latitudes, and a bunch of other things from the 2024 Census data of all U.S. counties. I'm not sure about the columns like LSAD or CBSAFP, I'm not sure what those stand for.**
...
**Task** Run the following code. Describe the output in the space below. **Answer - This filters to a specific state**
...
**Task** Run the following code. Describe the output in the space below. **Answer - It shows the actual counties in Maryland, but without their zipcodes. Just where each county is. But it doesn't show where the Chesapeake is at all.**
...
**Task** Run the following code. Describe the output in the space below. **Answer - This code layers the counties ON TOP OF the states, which shows where the Bay cuts through them. To correctly view this, however, the code for the county goes first -- I think of it as laying a foundation before building on it, because if you put the zip codes first then the counties sill cover them.**
...
## you layer from the ground up!! so the counties will be put first then the zipcodes
```
...
**Task** Run the following code. Describe the output in the space below. **Answer - This is the church data we've been using, so it has the differetn churches and where theyy are (city, state, zip, etc), as well as the other information.**
...
**Task** Run the following code. Describe the output in the space below. **Answer - This is like the previous task (the one after loading the data), except it filters it to just Maryland. It shows where the zipcodes cross with each county's bounds.**
...
**Task** Run the following code. Describe the output in the space below. How many churches are in Prince George's County? **Answer - There are 2120 churches in Prince George's county**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it tells us each zip code and the number of churches within them**
...
**Task** Run the following code. Describe the output in the space below. What spatial patterns do you notice? Which zip codes have the most churches? **Answer - The zip code with the most churches are near D.C. On the outer sections has the least amount of religious diversity (near Anne Arundel County) **
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer API KEY: f23e9b7c60f3c356678f297c9117fff98d3da6b0. ANSWER: the following code loads the libraries needed to observe the patterns of adherents across the state of Maryland (specifically in MD counties) using a choropleth map**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it shows FIPS codes, county names, number of adherents and the adherents as a percept of the population, and the largest congregation by number of how many adherents it has**
...
**Task** Run the following code. Describe the output in the space below. **Answer - The followign code pulls data from the 2017-2025 period from the Census website. Data for MD counties (for population) and the longitude and latitutde in each coutny in the state of Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it shows the number of congregations in each county, adherents and the percent of adherents as population, and the largest congregation by number of adherents in each county**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it shows the geometry for each county along with the county population -- geomety is basicaly just longitude and latitude**
...
**Task** Run the following code. Describe the output in the space below. **Answer - mainly the same things we have seen before but the information is joined with the adherents info and the gemoetry for the counties**
...
**Task** Run the following code. Describe the output in the space below. **Answer - it shows first ten counties, their populations, how many congregations and adherents they have as well as % of adherents for pop and which congregation is the biggest by the number of adherents, as well as the rate for them and the longitutde and latitude**
...
**Task** Run the following code. Describe the output in the space below. **Answer - a map of the state of MD with each county and rate of adherents(?) in each**
...
**Task** Run the following code. Describe the output in the space below. **Answer - this code shows the same thing as before but with clearer color**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer - this map gives the specific rates for each area in the state of maryland (county-wise), which zooms in on which counties have what in terms of differences in populations**
