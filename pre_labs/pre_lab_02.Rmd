...
**Answer is: Chi Alpha "University of Maryland" seems to potentially be a Greek church or religious group that is affiliated with UMD. They only have a PO box, indicating that they use the general address of the school.**
...
```{r}
md_churches |>
  filter(str_detect(NAME, "MUSLIM"))
```

**Answer is: I picked 'Muslim' because that is my starter for the Data Acquisition project.**
...
**Answer: Before, it was MM/DD/YR and now lubridate fixed it to be YR/MM/DD**
...
**Answer is: Using mutate and floor date, it anchors the code to the specific point in time when the money was moved around, which helps us narrow down the data we have.**
...
**It is showing us that the number of Catholics is steady growing but there aren't enough priests to lead cervices**
...
**Answer is: To be completely honest, I compared the before and after at least four times but didn't notice a change, but judging from the code I'm assuming it just arranged it in descending order.**
...
**Answer is: The number of parishes without a resident priet has steadily climbed over the time that the data was being collected.**
...
**Answer is: I asked ChatGPT, "for my data journalism class, what are some tidyverse functions similar to 'str_to-upper' and what do they do?' It showed me 'str_trim' to remove whitespace from the beginning and end of something input, and also 'str_to_title(x)', which capitalizes the first letter per word for a title.**
...
**Answer is: We basically swap information or names with the code. We're declaring that if it says one thing, it can also be read as whatever else we say.**
...
**Answer: Kind of. It makes everything more uniform but it still doesn't say what the money is actually for, so that's my only issue.**
...
1.  Is the donation from Richard Metzgar to Immanuel Lutheran Church recognized fully since it is listed as a political club and not a nonprofit or business organziation?  

2.  Can we filter which Catholic churches are in a five mile radius of each other?

3.  How can we filter churches throuhg 'AFFILIATION' to see which churches are the closest to one another with the same code?
