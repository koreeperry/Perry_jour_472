...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Yes, the zip ode gioven is not an actual part of the dataset.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **There set in the wrong data type. There are things that shouldn't be characters.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer is 56**
...
**Task** Run the following codeblock to filter the payments data to show only payments to vendors with "CHURCH" in their name. How many payments were made to churches? What was the total amount paid to churches? **The largest payment is from contracting the United Methodist Church for child care -- 60055531.56. **
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **The category data is not conclusive with the actual dataset for different years. (EX: 2013 has Grant, Loan and "L".**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **A grant for only $60 sticks out as unusual.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **It lowercases all of the category names.**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **It is making the capitalization of each category consistent so R can read the dataset properly.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **based on the number, I would say 58, but I'm not sure if that's compeltely correct. To be safe, there are at least TWo duplicates for each.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **21,197 rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **There is a new column that only has the zipcodes with five digits in it.**
