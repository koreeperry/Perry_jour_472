...
```{r}
adherents <- read_csv("tabula-Standard Denom Adherents.csv")
View(adherents)
...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer: So many. The category names are random numbers, most likely messed up points of data from the actual table we used. It seems that the first row of the data has risen to becoming the actual names of the columns. In addition to this, there are these random dots enxt to each state,  and each column has a different classification within R (either chr or dbl).**
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer: The names of each columns are still "x(number)."  Also, there are still spaces within the the percentage points, and there are still periods/dots after state abbreviations.*
...
**Task** Run the codeblock below. What else do we need to do? **Answer: The names of the categories are wrong. The dpercentages have spaces, and there are periods after the abbreviations of the states. Also, the types of categories switch between chr and dbl.**
...
##remember, with the rename command, the new name comes first and the old name comes second
```
...
**Task** Run the codeblock below. Look carefully at the results and identify and problems or issues **Answer: It fixes most of the data, but if the percentages have a space within them, they are fully cast out of the dataset.**  
...
**Task** Run the codeblock below. Look carefully at the results and identify and remaining problems or issues **This code makes it so the percentages in the data are actually put as values instead of being classified as N/A when there is a space between the numbers and the point.**  
...
##the code is basically saying (in terms of the periods next to the state abbreviations), remove all of the literal periods in the names and to remove all actual spaces
```
