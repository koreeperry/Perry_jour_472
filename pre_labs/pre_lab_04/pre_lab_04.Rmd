...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: There are many different ones that could be duplicates. Ex: on page 4, there are many grantees that say " Washington Co" with different tail titles like "Commissione" or "Commissioners" or Community A" or COmmunity Action Council".**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: Some of the fully uppercase grantees are switched, like 'Washington County Museum of Fine Arts'. It fully capitalized other titles that were not previously capitalized.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **I don't really see a difference in the data categroies**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: Definitely capitaliztion and organization of the data before and after we classigy that titles starting with "W" appear first.**
##ngrams shows of much fo something is inside of something else???
...
```{r}
clean_grants_and_loans <- read_csv("data/State-of-Maryland-Grant-and-Loan-Data-FY2009-to-FY2022-20250115-csv.csv")
```

## confused as to why my data wouldn't load